...
void
printPlayerBoard(struct set1D *F1, struct set1D *F2){
	coord current;
...
NextPlayerMove(coord pos, 
    ...
    coordCopy(set1DCandD.D1Array[0], pos);
    set1DCandD.length = 1;
...
int main()
{
    struct set1D setU, setT, F, F1, F2, F3, C, C1, C2;
    struct set2D setS, setP, setPowerSetS;
    int i = 0;
    ...
    while (!over) {
        ...
        NextPlayerMove(input, &F, &F1, &F2, &F3, over, &good, next, &setS, &C1, &C2, i);
        updateOver(&F3, &C1, &C2, &over, &setP);
        GameOver(&over, &next, &C1, &C2, &setP);
        printf("\n\n");
        printf("F1, F2:\n");
        printPlayerBoard(&F1, &F2);
        printf("F3 :\n");
        printGrid(F3);
        printf("C1, C2:\n");
        printPlayerBoard(&C1, &C2);
        printf("\n\n");
        i++;
    }
